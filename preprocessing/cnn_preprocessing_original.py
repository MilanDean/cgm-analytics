...
def create_gaf(sub_df, subject, label):
    ...
    plt.yticks([])
    plt.tight_layout()
    save_path = '../data/output/features/cnn/images/60min/'
    os.makedirs(save_path, exist_ok=True)
    filename = '{}_{}_{}.png'.format(subject, sub_df.timestamp.iloc[0].strftime(format='%Y%m%d_%H%M%S'), label)
    plt.savefig(os.path.join(save_path, filename), dpi=300)
    plt.close()
...
if __name__ == '__main__':
    ...
    # Read in featureset
    features = pd.read_csv('../data/output/features/60minWindow_val_set.csv')
    ...
    for subject in tqdm(all_data.subject.unique().tolist()):
        ...
        #sub_features = balance_onSubject(sub_features) # Already balanced
    ...
    os.makedirs('../data/output/features/CNN_60min/', exist_ok=True)
    full_featureset.to_csv('../data/output/features/synthetic_dataset_cnn_image_map_60minWindow_val.csv',
                           index = False)
